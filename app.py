...
def update_glaciers_text(selected_lat, selected_map, selected_p, selected_t):
    ...
    if selected_p is not None:
        ...
        dff = dff.loc[(dff.tstar_avg_prcpsol_mean_elev <= pmax) &
                      (dff.tstar_avg_prcpsol_mean_elev >= pmin)]
...
def make_precip_figure(selected_lat, selected_map, selected_p, selected_t):
    ...
    bins = np.arange(28) * 100

    pcp = df.tstar_avg_prcpsol_mean_elev.copy()
    pcp[~np.isfinite(pcp)] = 0
    hist1, edges1 = np.histogram(pcp, bins)
    scale = np.sum(hist1)
    hist1 = hist1 / scale

    pcp = dff.tstar_avg_prcpsol_mean_elev.copy()
    pcp[~np.isfinite(pcp)] = 0
    ...
    _layout['xaxis'] = dict(range=[0, 2700], title='Solid precipitation at avg. altitude (mm yr<sup>-1</sup>)')
